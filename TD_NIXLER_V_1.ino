...
// Global variables used for setting the time and time mode
int a = 0;
int b = 0;
int c = 0;
int d = 0;
int timeMODE = 24; // default mode = 24h 

int temperatureMode = 1;  // 1 = Celsius 
                          // 2 = Farenheit
...
void loop(){
 ...
 int YEA1 = now.year();
 Serial.println("-------------------------------------------------");
 Serial.println("NIXLER by Tor Design - Software version: 1.0");
 Serial.print("Time: ");
 Serial.print(HOU1);
 Serial.print(":");
 Serial.print(MIN1);
...
int setupNixler(int Btn1, int Btn2){
      ...
      while(digitalRead(Btn2) == LOW){
              ...
              if(STATE == true){ // If Fahrenheit set to Celsius
                    temperatureMode = 1;
                    dispNixler(10,temperatureMode,temperatureMode,10);     
              }else{
                    temperatureMode = 2;
...
int checkButtons(){
     ...
     if(Btn1_state == LOW && Btn2_state == HIGH){
          cycleNixler(3); // Put on a show... new function goes here blinky, blinky, party, party.
     }
